...
module.exports = {
        ...
        (err, row) => {
          ...
          if (row[0 === undefined]) {
            return res
              .status(404)
              .send(
                "<script>alert('아이디가 존재하지 않습니다.');history.go(-1);</script>"
              );
          }
          ...
          if (!match) {
            return res
              .status(404)
              .send(
                "<script>alert('아이디 혹은 비밀번호가 일치하지 않습니다.');history.go(-1)</script>"
              );
          }
          try {
            //access Token 발급
            ...
            pool.query(
              `UPDATE user SET login_date = "${current}" WHERE id = "${id}"`
            );
            if (
  ...
  signup: (req, res) => {
    ...
    try {
      pool.query(
        'SELECT id FROM user WHERE id = "' + user_info.id + '"',
        (err, row) => {
          if (row[0] == undefined) {
            //동일한 아이디가 없을 경우
          ...
          `;
            const data = [
              ...
              user_info.nickname,
              user_info.authority,
              user_info.required_check1,
              user_info.required_check2,
              user_info.optional_check1,
              user_info.optional_check2,
              user_info.info_period,
            ];
            pool.query(sql, data, (err, rows, fields) => {
              ...
              request.post(options, function (err, httpResponse, body) {
                console.log('회원가입 슬랙 웹훅 : ' + body);
              });
              return res
                .status(200)
                .send(
                  "<script>alert('회원가입이 완료되었습니다. 로그인하신 후 서비스를 이용해주세요');location.href = '/login';</script>"
                );
            });
          } else {
            ...
            res
              .status(400)
              .send(
                "<script>alert('중복된 아이디가 있습니다.');location.href = document.referrer;</script>"
              );
          }
        }
      );
  ...
  duplic: (req, res) => {
    ...
    try {
      if (check == 'id') {
        ...
        if (check == 'nickname') {
          pool.query(
            'SELECT nickname FROM user WHERE nickname = "' + checkName + '"',
            (err, row) => {
              if (row[0] == undefined) {
                return res.status(200).send(JSON.stringify(result));
              } else {
  ...
  mailAuthPw: (req, res) => {
    ...
    let emailTemplate;
    ejs.renderFile(
      rootdir + '/template/autoMailPw.ejs',
      { authCode: authNum },
      function (err, data) {
        if (err) {
          console.log(err);
      ...
      pool.query(sql, (err, row) => {
        if (err) return res.status(500).json(err);
            ...
            pool.query(updateSql, (err, row) => {
              if (err) return res.status(500).json(err);
                ...
                return res
                  .status(200)
                  .send(
                    "<script>alert('비밀번호 변경이 완료되었습니다.');location.href='/mypage/profile';</script>"
                  );
              }
            });
          } else {
          ...
          return res
            .status(400)
            .send(
              "<script>alert('찾을 수 없는 정보입니다. 다시 로그인해주세요.').location.href='/login';</script>"
            );
        }
      });
    } catch (err) {
  ...
  resetPw: (req, res) => {
    ...
    if (id == '') {
      return res
        .status(200)
        .send(
          "<script>alert('오류가 발생했습니다. 다시 시도해주세요.');location.href='/login';<script>"
        );
    }
    try {
      pool.query(updateSql, (err, row) => {
        if (err) return res.status(500).json(err);
          ...
          return res
            .status(200)
            .send(
              "<script>alert('비밀번호가 변경이 되었습니다.');location.href='/login';</script>"
            );
        }
      });
    } catch (err) {
      ...
      pool.query(sql, (err, row) => {
        if (err) return res.status(500).json(err);
          ...
          return res
            .status(200)
            .send(
              "<script>alert('회원정보 변경이 완료되었습니다.');location.href='/mypage/profile';</script>"
            );
        }
      });
    } catch (er) {
      ...
      pool.query(sql, (err, row) => {
        if (err) return res.status(500).json(err);
          ...
          return res
            .status(200)
            .send(
              "<script>alert('경력정보 변경이 완료되었습니다.');location.href='/mypage/profile';</script>"
            );
        }
      });
    } catch (err) {
      ...
      pool.query(sql, data, (err, row) => {
        if (err) return res.status(500).json(err);
          ...
          return res
            .status(200)
            .send(
              "<script>alert('경력정보 추가가 완료되었습니다.');location.href='/mypage/profile';</script>"
            );
        }
      });
    } catch (err) {
  ...
  userData: (req, res) => {
    ...
    if (data.info.length > 0) {
      return res
        .status(200)
        .render(rootdir + '/public/mypage_profile.html', data);
    } else {
